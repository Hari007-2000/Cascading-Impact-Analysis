...
def _norm_col(label) -> str:
    """Like _norm, but first drops any parenthetical unit, e.g. 'EXPORTS (kg/yr)'."""
...
def _find_col(df: pd.DataFrame, tokens: set[str]):
    ...
    Matching ignores a trailing unit in parentheses, so 'EXPORTS (kg/yr)' and
    'FINAL_DEMAND' both resolve correctly.
    """
    for c in df.columns:
        if _norm_col(c) in tokens:
            return c
    return None


# --------------------------------------------------------------------------- #
# Default final-demand fallback (bundled APAP_Exports_16.csv)
# --------------------------------------------------------------------------- #
def load_fd_fallback(source) -> pd.DataFrame:
    """
    Load a default final-demand table (e.g. the bundled APAP_Exports_16.csv).

    Expects a commodity column plus a FINAL_DEMAND column and, optionally, an
    EXPORTS column. Returns a DataFrame indexed by commodity with numeric
    columns 'FINAL_DEMAND' and 'EXPORTS' (the latter zero-filled if absent).
    This is used to supply a baseline final demand when an uploaded PIOT has no
    final-demand column of its own.
    """
    fb = pd.read_csv(source, index_col=0)
    fb.index = [str(i).strip() for i in fb.index]
    fd_col = _find_col(fb, _FINAL_DEMAND_TOKENS)
    exp_col = _find_col(fb, _EXPORTS_TOKENS)
    out = pd.DataFrame(index=fb.index)
    out["FINAL_DEMAND"] = (pd.to_numeric(fb[fd_col], errors="coerce").fillna(0.0)
                           if fd_col is not None else 0.0)
    out["EXPORTS"] = (pd.to_numeric(fb[exp_col], errors="coerce").fillna(0.0)
                      if exp_col is not None else 0.0)
    return out


def _match_fallback(fallback: pd.DataFrame, industries: list[str]):
    """
    Align a fallback final-demand table to the PIOT industries.

    Matching is by normalised name first (case/punctuation-insensitive); any
    industry still unmatched is filled POSITIONALLY when the fallback has exactly
    the same number of rows as there are industries (handles abbreviated PIOT
    labels like ACETANH ↔ Acetic_Anhydride that share the same ordering).

    Returns (fd_array, exp_array, matched_count).
    """
    n = len(industries)
    fd = np.full(n, np.nan)
    exp = np.zeros(n, dtype=float)
    by_norm = {}
    for label in fallback.index:
        by_norm.setdefault(_norm(label), label)
    ...
    for k, ind in enumerate(industries):
        key = _norm(ind)
        if key in by_norm:
            row = fallback.loc[by_norm[key]]
            fd[k] = float(row["FINAL_DEMAND"])
            exp[k] = float(row["EXPORTS"])
            matched += 1
        else:
            unmatched.append(k)

    if unmatched and len(fallback) == n:
        for k in unmatched:
            row = fallback.iloc[k]
            fd[k] = float(row["FINAL_DEMAND"])
            exp[k] = float(row["EXPORTS"])
...
def build_network(df: pd.DataFrame, industries: list[str],
                  ...
                  fd_fallback: pd.DataFrame | None = None) -> dict:
    """
